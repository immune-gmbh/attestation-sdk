...
func (stor *Storage) InsertAnalyzeReport(ctx context.Context, report *models.AnalyzeReport) (retErr error) {
	...
	tx, err := stor.DB.BeginTx(ctx, nil)
	if err != nil {
	...
	for idx := range report.AnalyzerReports {
		...
		err := stor.insertAnalyzerReport(tx, analyzerReport)
		if err != nil {
...
func (stor *Storage) insertAnalyzerReport(tx *sql.Tx, report *models.AnalyzerReport) error {
	...
	report.ID = uint64(lastID)
	err = stor.aoscWorkaroundBuildAnalyzerReportIndexes(tx, report)
	if err != nil {
...
func (stor *Storage) newTx() (*sqlx.Tx, context.CancelFunc, error) {
