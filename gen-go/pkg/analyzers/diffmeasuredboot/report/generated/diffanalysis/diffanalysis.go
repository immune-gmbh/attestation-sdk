...
func (p DiffDiagnosis) String() string {
  switch p {
  case DiffDiagnosis_Undefined: return "Undefined"
  case DiffDiagnosis_Match: return "Match"
  case DiffDiagnosis_UnsuspiciousDamage: return "UnsuspiciousDamage"
  case DiffDiagnosis_SuspiciousDamage: return "SuspiciousDamage"
  case DiffDiagnosis_FirmwareVersionMismatch: return "FirmwareVersionMismatch"
  case DiffDiagnosis_InvalidOriginalFirmware: return "InvalidOriginalFirmware"
  case DiffDiagnosis_KnownTamperedHost: return "KnownTamperedHost"
  }
  return "<UNSET>"
}
...
func DiffDiagnosisFromString(s string) (DiffDiagnosis, error) {
  switch s {
  case "Undefined": return DiffDiagnosis_Undefined, nil 
  case "Match": return DiffDiagnosis_Match, nil 
  case "UnsuspiciousDamage": return DiffDiagnosis_UnsuspiciousDamage, nil 
  case "SuspiciousDamage": return DiffDiagnosis_SuspiciousDamage, nil 
  case "FirmwareVersionMismatch": return DiffDiagnosis_FirmwareVersionMismatch, nil 
  case "InvalidOriginalFirmware": return DiffDiagnosis_InvalidOriginalFirmware, nil 
  case "KnownTamperedHost": return DiffDiagnosis_KnownTamperedHost, nil 
  }
...
func (p *RangeOrForcedData) Equals(other *RangeOrForcedData) bool {
  ...
  if !p.Range.Equals(other.Range) { return false }
  if bytes.Compare(p.ForceData, other.ForceData) != 0 { return false }
  return true
}
...
func (p *Measurement) Equals(other *Measurement) bool {
  ...
  if p.ID != other.ID { return false }
  if len(p.DataChunks) != len(other.DataChunks) { return false }
...
func (p *RelatedMeasurement) Equals(other *RelatedMeasurement) bool {
  ...
  if !p.Measurement.Equals(other.Measurement) { return false }
  if len(p.RelatedDataChunks) != len(other.RelatedDataChunks) { return false }
...
func (p *DiffEntry) Equals(other *DiffEntry) bool {
  ...
  if p.OBSOLETE_Start != other.OBSOLETE_Start { return false }
  if p.OBSOLETE_Length != other.OBSOLETE_Length { return false }
  if p.HammingDistance != other.HammingDistance { return false }
  if p.HammingDistanceNon00orFF != other.HammingDistanceNon00orFF { return false }
  if !p.Range.Equals(other.Range) { return false }
  if len(p.RelatedMeasurements) != len(other.RelatedMeasurements) { return false }
...
func (p *CustomReport) Equals(other *CustomReport) bool {
  ...
  if p.Diagnosis != other.Diagnosis { return false }
  if len(p.DiffEntries) != len(other.DiffEntries) { return false }
  for i, _tgt := range p.DiffEntries {
    _src9 := other.DiffEntries[i]
    if !_tgt.Equals(_src9) { return false }
  }
  if p.ImageOffset != other.ImageOffset { return false }
  return true
}
