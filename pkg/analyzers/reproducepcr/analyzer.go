...
type Input struct {
	...
	AlignedOriginalFirmware analysis.AlignedOriginalFirmware
...
func (analyzer *ReproducePCR) Analyze(ctx context.Context, in Input) (*analysis.Report, error) {
	...
	var biosImg *biosimage.BIOSImage
	if fw, err := uefi.ParseUEFIFirmwareBytes(in.ActualFirmware.Bytes()); err == nil { // TODO: parse the image in analysis.calculators, this is a very slow procedure and should be cached.
		biosImg = biosimage.NewFromParsed(fw)
	} else {
		report.Issues = append(report.Issues, analysis.Issue{
			Severity:    analysis.SeverityWarning,
			Description: fmt.Sprintf("Unable to parse the actual image, using the original image to get the measurements: %v", err),
		})
		biosImg = biosimage.NewFromParsed(in.OriginalFirmware.UEFI())
	}
